...
namespace X.Neo4j.AspNetCore.Identity.Tests
{
    ...
    public class UserStoreTests
    {
        ...
        private IdentityUser FetchTestUser(IGraphClient client, UserStore<IdentityUser> us)
        {
        ...
        private void Clean(IGraphClient client)
        {
            client.Cypher.Match("(n:IdentityRole)").Where("n.Name= 'TEST_ROLE'").DetachDelete("n")
                .ExecuteWithoutResultsAsync().Wait();
            client.Cypher.Match("(n:IdentityUser)").Where("n.Email_Value= 'test@testuser.com'").DetachDelete("n")
                .ExecuteWithoutResultsAsync().Wait();
            client.Cypher.Match("(n:IdentityClaim)").Where("n.Type= 'TEST_CLAIM'").DetachDelete("n")
                .ExecuteWithoutResultsAsync().Wait();
            client.Cypher.Match("(n:IdentityExternalLogin)").Where("n.LoginProvider= 'GOOGLE'").DetachDelete("n")
                .ExecuteWithoutResultsAsync().Wait();
        }

        private IdentityUser CreateUser(IGraphClient client, UserStore<IdentityUser> us)
        {
